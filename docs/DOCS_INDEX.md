...
| [Releasing](../RELEASING.md) | Manual core release: the single workspace version, `scripts/bump-version.sh` (pin only), `workflow_dispatch` on `release-core.yml`, post-publication Docker/Homebrew via `publish-core-channels.yml`, and China asset mirror via `sync-release-to-cnb.yml`. |
