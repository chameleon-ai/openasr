...
use super::dtw_word_timestamps::{
    WHISPER_DTW_ENVELOPE_FRAME_COUNT, WHISPER_DTW_ONSET_FLOOR_MARGIN_DB,
    WHISPER_DTW_ONSET_SUSTAIN_FRAMES, WhisperWordTimestampMode, audio_duration_seconds,
    run_frame_bounds, text_token_runs, whisper_cross_attention_word_timestamps,
    whisper_decoder_cross_attention_flags, whisper_dtw_word_audio_rms_frames,
    whisper_word_timestamp_mode,
};
...
struct WhisperDecoderActorJob {
    ...
    decoder_state: Seq2SeqDecoderState,
    audio_duration: f32,
    /// Per-0.02-s-frame RMS envelope of the request audio (buffer-absolute, in
    /// the whisper 16 kHz / 0.02 s frame units the DTW word times use), `None`
    /// when the run did not request cross-attention word timestamps. Feeds the
    /// pause-boundary word-onset refinement so a word the center fold parked in
    /// a mid-segment silence is pulled to its real audio onset.
    word_audio_rms_frames: Option<Vec<f32>>,
    ...
    fn run(
        ...
        let result = (|| {
            ...
            run_whisper_decode_loop(
                ...
                &encoder_result,
                self.audio_duration,
                self.word_audio_rms_frames.as_deref(),
...
#[derive(Debug, Clone, PartialEq)]
pub(crate) struct WhisperGeneratedTokenAlignment {
    pub(crate) token_id: u32,
    pub(crate) frame_probs: Vec<f32>,
}
...
fn execute_whisper_with_prepared_runtime(
    ...
    let word_audio_rms_frames = whisper_dtw_word_audio_rms_frames(prepared_audio, request_options);
    ...
    if let Some(unified_actor) = unified_actor {
        ...
        let mut decoder_job = WhisperDecoderActorJob {
            ...
            decoder_state,
            audio_duration,
            word_audio_rms_frames: word_audio_rms_frames.clone(),
            allow_persistent_session_reuse,
